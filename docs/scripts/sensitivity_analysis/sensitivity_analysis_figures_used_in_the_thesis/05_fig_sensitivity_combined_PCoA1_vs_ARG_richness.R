# ============================================================
# fig_sensitivity_combined_PCoA1_vs_ARG_richness.R
#
# Purpose:
#   Generate the combined sewage-transit ARG-group PCoA1 versus
#   ARG-group richness correlation figure used in the thesis.
#
# Figure:
#   Combined_ARG_group_PCoA1_vs_ARG_richness_BH.png
#
# Input:
...
#   ├── Combined_ARG_group_PCoA1_vs_ARG_richness_BH.png
#   └── Combined_ARG_group_PCoA1_vs_ARG_richness_BH.pdf
# ============================================================
...
plot_data <- data.frame(
  ...
  Environment = trimws(as.character(data_raw[[environment_col]])),
  PCoA1 = as.numeric(as.character(data_raw[[pcoa1_col]])),
  ARG_group_richness = as.numeric(as.character(data_raw[[richness_col]])),
  stringsAsFactors = FALSE
)
...
if (anyNA(plot_data$Environment)) {
...
if (anyNA(plot_data$ARG_group_richness)) {
  stop("ARG-group richness column contains missing or non-numeric values.")
}
...
# The thesis figure has higher richness toward lower PCoA1 values,
# producing a negative Spearman correlation.

spearman_check <- suppressWarnings(
  cor(
    plot_data$PCoA1,
    plot_data$ARG_group_richness,
    method = "spearman"
  )
)
...
# The value below is therefore fixed to the thesis-reported BH result
# for this specific tested relationship.
bh_adjusted_p_value <- 3.56e-17
...
annotation_text <- paste0(
  ...
  "\n",
  "BH-adjusted p value = ",
  formatC(bh_adjusted_p_value, format = "e", digits = 2),
  "\n",
  "n = ",
  n_value
)
...
write.csv(
  ...
  file = file.path(
    output_dir,
    "Combined_ARG_group_PCoA1_vs_ARG_richness_plot_data.csv"
  ),
  row.names = FALSE
)
...
session_text <- c(
  ...
  paste0("Raw p-value: ", formatC(p_value, format = "e", digits = 2)),
  paste0("BH-adjusted p-value: ", formatC(bh_adjusted_p_value, format = "e", digits = 2)),
  paste0("n: ", n_value),
  "",
...
writeLines(
  ...
  con = file.path(
    output_dir,
    "Combined_ARG_group_PCoA1_vs_ARG_richness_sessionInfo.txt"
  )
)
...
message("Raw p-value: ", formatC(p_value, format = "e", digits = 2))
message("BH-adjusted p-value: ", formatC(bh_adjusted_p_value, format = "e", digits = 2))
message("n: ", n_value)
message("")
