# ============================================================
# fig_sensitivity_nestedness_component_boxplot.R
#
# Purpose:
#   Generate the ARG-group nestedness-resultant component
#   boxplot for the Jaccard decomposition sensitivity analysis.
#
# Figure:
#   ARG_group_nestedness_component_boxplot.png
#
# Input:
...
#   ├── ARG_group_nestedness_component_boxplot.png
#   └── ARG_group_nestedness_component_boxplot.pdf
# ============================================================
...
plot_data <- data.frame(
  ...
  Nestedness_resultant_component =
    as.numeric(as.character(decomposition_raw[[nestedness_col]])),
  stringsAsFactors = FALSE
)
...
if (anyNA(plot_data$Nestedness_resultant_component)) {
...
write.csv(
  ...
  file = file.path(
    output_dir,
    "ARG_group_nestedness_component_boxplot_plot_data.csv"
  ),
  row.names = FALSE
)
...
summary_table <- do.call(
  ...
  lapply(
    ...
    function(current_pair_type) {
      x <- plot_data$Nestedness_resultant_component[
...
write.csv(
  ...
  file = file.path(
    output_dir,
    "ARG_group_nestedness_component_boxplot_summary.csv"
  ),
  row.names = FALSE
)
...
p <- ggplot(
  ...
  aes(
    x = Pair_type,
    y = Nestedness_resultant_component,
    fill = Pair_type,
    colour = Pair_type
  )
) +
  ...
  labs(
    x = "Pair type",
    y = "Nestedness-resultant component"
  ) +
...
session_text <- c(
  ...
  "",
  "Figure: nestedness-resultant component boxplot by pair type",
...
writeLines(
  ...
  con = file.path(
    output_dir,
    "ARG_group_nestedness_component_boxplot_sessionInfo.txt"
  )
)
