# ============================================================
# fig_sensitivity_total_jaccard_dissimilarity_boxplot.R
#
# Purpose:
#   Generate the ARG-group total Jaccard dissimilarity boxplot
#   for the Jaccard decomposition sensitivity analysis.
#
# Figure:
#   ARG_group_total_Jaccard_dissimilarity_boxplot.png
#
# Input:
...
#   ├── ARG_group_total_Jaccard_dissimilarity_boxplot.png
#   └── ARG_group_total_Jaccard_dissimilarity_boxplot.pdf
# ============================================================
...
plot_data <- data.frame(
  ...
  Total_Jaccard_dissimilarity =
    as.numeric(as.character(decomposition_raw[[total_jaccard_col]])),
  stringsAsFactors = FALSE
)
...
if (anyNA(plot_data$Total_Jaccard_dissimilarity)) {
  stop("Total Jaccard dissimilarity contains missing or non-numeric values.")
}
...
# ------------------------------------------------------------
# 5. Save plotting data and summary table
# ------------------------------------------------------------
...
write.csv(
  ...
  file = file.path(
    output_dir,
    "ARG_group_total_Jaccard_dissimilarity_boxplot_plot_data.csv"
  ),
  row.names = FALSE
)
...
summary_table <- do.call(
  ...
  lapply(
    ...
    function(current_pair_type) {
      x <- plot_data$Total_Jaccard_dissimilarity[
...
write.csv(
  ...
  file = file.path(
    output_dir,
    "ARG_group_total_Jaccard_dissimilarity_boxplot_summary.csv"
  ),
  row.names = FALSE
)
...
session_text <- c(
  ...
  "",
  "Figure: total Jaccard dissimilarity boxplot by pair type",
...
writeLines(
  ...
  con = file.path(
    output_dir,
    "ARG_group_total_Jaccard_dissimilarity_boxplot_sessionInfo.txt"
  )
)
