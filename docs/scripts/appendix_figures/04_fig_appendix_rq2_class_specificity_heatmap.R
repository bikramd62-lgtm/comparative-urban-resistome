# ============================================================
# fig_appendix_rq2_class_specificity_heatmap.R
#
# Purpose:
# Generate the appendix RQ2 ARG Drug Class focused specificity
# heatmap.
#
# The heatmap shows binary detection of ARG Drug Classes selected
# from the strongest descriptive prevalence differences between
# transit and sewage.
#
...
# results/rq2/prevalence_difference/rq2_class_prevalence_comparison.tsv
#
# Output:
# results/appendix_figures/appendix_rq2_class_specificity_heatmap.png
# results/appendix_figures/appendix_rq2_class_specificity_heatmap.pdf
# results/appendix_figures/appendix_rq2_class_specificity_heatmap_plotdata.tsv
# results/appendix_figures/appendix_rq2_class_specificity_heatmap_selected_features.tsv
# ============================================================
...
prevalence_file <- file.path(
  ...
  "rq2_class_prevalence_comparison.tsv"
)
...
# The appendix figure shows the 15 ARG Drug Classes with the
# strongest descriptive difference toward sewage and the 15
# strongest descriptive difference toward transit.
...
# ------------------------------------------------------------
# 5. Read ARG Drug Class binary matrix
# ------------------------------------------------------------
...
if (ncol(raw_matrix) < 2) {
  stop(
    "The binary matrix must contain one profile column and at least one ARG Drug Class column."
  )
}
...
if (any(is.na(binary_matrix))) {
  stop(
    "The ARG Drug Class binary matrix contains missing or non-numeric values."
  )
}
...
# ------------------------------------------------------------
# 8. Prepare ARG Drug Class labels
# ------------------------------------------------------------
...
if (nrow(selected_features) == 0) {
  stop(
    "No ARG Drug Classes were selected for plotting. Check the prevalence-difference table and binary matrix."
  )
}
...
# ------------------------------------------------------------
# 13. Generate final appendix figure
# ------------------------------------------------------------
...
png_output <- file.path(
  output_dir,
  "appendix_rq2_class_specificity_heatmap.png"
)

pdf_output <- file.path(
  output_dir,
  "appendix_rq2_class_specificity_heatmap.pdf"
)

plot_data_output <- file.path(
  output_dir,
  "appendix_rq2_class_specificity_heatmap_plotdata.tsv"
)

selected_features_output <- file.path(
  output_dir,
  "appendix_rq2_class_specificity_heatmap_selected_features.tsv"
)
