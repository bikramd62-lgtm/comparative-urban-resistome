# ============================================================
# fig_appendix_rq1_class_presence_absence_heatmap.R
#
# Purpose:
# Generate the appendix Resistance Class presence/absence heatmap
# for the RQ1 comparative resistome analysis.
#
# Input:
# results/rq1/matrices/rq1_class_combined_binary_matrix.tsv
#
# Output:
# results/appendix_figures/appendix_rq1_class_presence_absence_heatmap_navy_darkergrey_border.png
# results/appendix_figures/appendix_rq1_class_presence_absence_heatmap_navy_darkergrey_border.pdf
# ============================================================
...
if (!file.exists(input_file)) {
  stop(
    paste0(
      ...
      input_file,
      "\nRun 01_build_rq1_binary_matrices.R before generating this appendix figure."
    )
  )
}

# ------------------------------------------------------------
# 3. Read combined Resistance Class binary matrix
# ------------------------------------------------------------
...
if (ncol(raw_df) < 2) {
  stop(
    "Input matrix must contain one profile column and at least one Resistance Class column."
  )
}
...
if (any(is.na(binary_matrix))) {
  stop(
    "The Resistance Class binary matrix contains missing or non-numeric values."
  )
}
...
if (any(duplicated(rownames(binary_matrix)))) {
  stop(
    "Duplicated profile names detected in the Resistance Class matrix."
  )
}

if (any(duplicated(colnames(binary_matrix)))) {
  stop(
    "Duplicated Resistance Class names detected in the Resistance Class matrix."
  )
}
...
# ------------------------------------------------------------
# 5. Order Resistance Classes by total detection frequency
# ------------------------------------------------------------
...
features_ordered_for_plot <- feature_order_table$Feature

# ------------------------------------------------------------
# 6. Helper functions for display labels
# ------------------------------------------------------------

make_class_label <- function(feature_name) {

  label <- feature_name

  # If MEGARes-style labels contain comma-separated hierarchy,
  # use the final element as the short Resistance Class label.
  label <- sub(
    "^.*,",
    "",
    label
...
feature_label_table <- data.frame(
  Feature = features_ordered_for_plot,
  ARG_Drug_Class_label = vapply(
    features_ordered_for_plot,
    make_class_label,
...
if (any(duplicated(feature_label_table$ARG_Drug_Class_label))) {
  feature_label_table$ARG_Drug_Class_label <- make.unique(
    feature_label_table$ARG_Drug_Class_label,
    sep = "_"
  )
}
...
heatmap_df$ARG_Drug_Class_label <- factor(
  heatmap_df$ARG_Drug_Class_label,
  levels = feature_label_table$ARG_Drug_Class_label
)
...
# ------------------------------------------------------------
# 8. Generate final appendix figure
# ------------------------------------------------------------
...
png_output <- file.path(
  output_dir,
  "appendix_rq1_class_presence_absence_heatmap_navy_darkergrey_border.png"
)

pdf_output <- file.path(
  output_dir,
  "appendix_rq1_class_presence_absence_heatmap_navy_darkergrey_border.pdf"
)

plot_data_output <- file.path(
  output_dir,
  "appendix_rq1_class_presence_absence_heatmap_plotdata.tsv"
)

selected_features_output <- file.path(
  output_dir,
  "appendix_rq1_class_presence_absence_heatmap_selected_features.tsv"
)
...
message(
  "\nDisplayed Resistance Classes saved:"
)
